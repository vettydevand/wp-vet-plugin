...
*   [x] **3.5. Riscritto il file JS per allinearlo alla nuova logica, gestire il modale, usare i dati corretti e le stringhe localizzate.**
...
*   [ ] **6.0. Adesione alle Linee Guida:** Assicurarsi di seguire costantemente le regole definite in `DEVELOPER_GUIDELINES.md`.
...
*   [ ] **6.3. Analisi del `package.json` del monorepo:** Leggere il `package.json` originale per identificare tutte le dipendenze di FullCalendar e gli script di build rilevanti.
*   [ ] **6.4. Creazione `package.json` dedicato e pulizia:** Rimuovere i file di configurazione del monorepo (`package.json`, `pnpm-lock.yaml`, `pnpm-workspace.yaml`, `turbo.json`) e inizializzare un nuovo `package.json` pulito.
*   [ ] **6.5. Installazione dipendenze via `pnpm`:** Installare le versioni corrette di `@fullcalendar/core`, `@fullcalendar/daygrid`, `@fullcalendar/timegrid`, `@fullcalendar/list`, `@fullcalendar/interaction` e tutte le dipendenze di sviluppo necessarie (es. `webpack`, `babel`, `css-loader`, etc.).
*   [ ] **6.6. Configurazione processo di build:** Creare un file `webpack.config.js` per definire come compilare e raggruppare i file JavaScript e CSS in bundle ottimizzati.
*   [ ] **6.7. Integrazione nel plugin WordPress:** Modificare il plugin PHP per caricare i nuovi bundle (`bundle.js`, `bundle.css`) invece delle vecchie risorse.
*   [ ] **6.8. Pulizia del repository:** Rimuovere i file e le cartelle del monorepo originale di FullCalendar non più necessari.
*   [ ] **6.9. Test e validazione:** Verificare che il calendario funzioni come prima con la nuova struttura.
*   [ ] **6.10. Merge del branch:** Unire le modifiche nel branch `main` una volta completato e testato il refactoring.
